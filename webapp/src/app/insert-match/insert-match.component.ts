...
export class InsertMatchComponent implements OnInit, OnChanges {
    ...
    validate() : boolean {
        ...
        if (this.match.team1score === this.match.team2score && this.match.isFinal) {
            if (this.match.team1penalties == null || this.match.team1penalties.length === 0) {
                this.error = { description: 'Verifique os pênaltis dos times.' };
                return false;
            }

            if (this.match.team2penalties == null || this.match.team2penalties.length === 0) {
                this.error = { description: 'Verifique os pênaltis dos times.' };
                return false;
            }
    ...
    tryCreateMatch() {
        ...
        this.matchService.getFinalFromChampionship(this.match.championship._id).toPromise().then(response => {
            ...
            request.subscribe((result) => {
                this.matchCreatedSuccess(result);
            }, (error : any) => {
                console.log(error);
                this.error = error;
            });
    ...
    private getDefaultPickerOptions() {
        return {
            ...
            weekdaysFull: ['Domingo', 'Segunda-feira', 'Terça-feira',
