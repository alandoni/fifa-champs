...
export class TeamPickComponent implements OnInit {
    ...
    chooseTeams() {
        const badgePlaceholder = 'https://cdn.tutsplus.com/net/uploads/legacy/958_placeholders/placehold.gif';

        const indexTeamOne = this.randomIntFromInterval(1, this.jsonTeams[this.stars].length) - 1;
        this.team1 = this.jsonTeams[this.stars][indexTeamOne];
        this.team1.badgeImage = (this.team1.badgeImage === '') ? badgePlaceholder : this.team1.badgeImage;
        this.jsonTeams[this.stars].splice(indexTeamOne, 1);

        const indexTeamTwo = this.randomIntFromInterval(1, this.jsonTeams[this.stars].length) - 1;
        this.team2 = this.jsonTeams[this.stars][indexTeamTwo];
        this.team2.badgeImage = (this.team2.badgeImage === '') ? badgePlaceholder : this.team2.badgeImage;
