...
export class CreatePlayerComponent implements OnInit, OnChanges {
    ...
    tryCreatePlayer() {
        ...
        this.playerService.insert(this.nickname).subscribe((result) => {
            this.playerCreatedSuccess(result);
        }, (error) => {
    ...
    tryUpdatePlayer() {
        this.playerService.update(this.player._id, this.nickname, null).subscribe((result) => {
            this.playerCreatedSuccess(result);
        }, (error) => {
            console.log(<any>error);
            this.error = <any>error;
